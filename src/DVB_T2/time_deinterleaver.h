...
private:
    ...
    std::vector<int> fec_len_bits{};              // FEC block size (16200 or 64800)
    std::vector<int> bits_per_cell{};             // constellation order
    std::vector<int> n_ti{};                      // number of TI blocks per Interleaving Frame
    std::vector<int> p_i{};                       // number of T2 Frame per Interleaving Frame
    std::vector<int> frame_interval{};            // jump to next T2 Frame for Interleaving Frame
    std::vector<int> first_frame_idx{};           // index of the first T2 Frame of the super-frame for current PLP
    std::vector<int> cells_per_fec_block{};       // number of cell per FEC block
    std::vector<int> slice_end{};                 // the address of the last cell occupied by a common or Type 1 PLP
    std::vector<int> num_rows{};                  // number of rows per TI block
    std::vector<std::vector<int>> num_cols{};     // number of colums per TI block
    std::vector<std::vector<int>> permutations{}; // address cell deinterleaving
    std::vector<int> last_frame_idx{};            // last T2 frame of first interleaving frame
    ...
    std::vector<std::vector<int>>
        fec_blocks_per_time_interleving{};        // FEC block in each time interleaving
